...
tilt_model = models.Sequential([
    layers.Flatten(input_shape=(8,)),
    layers.Dense(30, activation='relu'),
    layers.Dense(1, activation='sigmoid')
])
tilt_model.compile(optimizer='adam',
                   loss='sparse_categorical_crossentropy',
                   metrics=['accuracy'])

# -------------------------------checkpoints-------------------------------
# checkpoints are in rocket_v2, they are mostly unused
if loading_model:
    engine_model = models.load_model('rocket_v3_engine.h5')
    tilt_model = models.load_model('rocket_v3_tilt.h5')
...
class Rocket:
    ...
    def fly(self):
        ...
        tilt_prediction = tilt_model.predict(np.array([squished_state]))

        predictions.append(engine_prediction[0][0])  # accessing engine value from 2d array given
        predictions.append(tilt_prediction[0][0])
...
def train(inputs, outputs):
    ...
    # splitting outputs into 2 arrays
    engine_outputs = np.array([])
    tilt_outputs = np.array([])
    for output in outputs:
        engine_outputs = np.append(engine_outputs, output[0])
        tilt_outputs = np.append(tilt_outputs, output[1])

    engine_model.fit(inputs, engine_outputs, epochs=10, verbose=1)
    tilt_model.fit(inputs, tilt_outputs, epochs=10, verbose=1)
    # add callbacks=[checkpoint_callback] for checkpoints
    if saving_model:
        engine_model.save('rocket_v3_engine.h5')
        tilt_model.save('rocket_v3_tilt.h5')
